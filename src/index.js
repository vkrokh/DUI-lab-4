...
function loadSources() {
    const url = 'https://newsapi.org/v2/sources?apiKey=1b53ce10fb9640c4a0d21e720a3901ec';
    ...
    fetch(request)
        ...
        .then(function (data) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                opt.innerHTML = data.sources[i].name;
...
function createNewsItem(token, data) {
    ...
    token.querySelector('.news__img').src = data.urlToImage;
    token.querySelector('.news__text').innerHTML = data.description;
...
function loadBy(urlPart) {
    hide('#error-block');
    const url = 'https://newsapi.org/v2/' + urlPart + 'apiKey=1b53ce10fb9640c4a0d21e720a3901ec';
    const request = new Request(url);
    fetch(request)
        .then(function (response) {
            return response.json();
